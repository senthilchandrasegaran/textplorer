...
var words = [], max, scale = 1, complete = 0, keyword = '', tags, fontSize,
    maxLength = 30, numLines = 0;
...
var speakerColors = ['#a6cee3', '#fb9a99', '#b2df8a', '#bc80bd'];
...
// interface.
var colorlistFull = [
    'rgba(177,89,40,',   'rgba(106,61,154,',  'rgba(202,178,214,',
    'rgba(253,191,111,', 'rgba(227,26,28,',   'rgba(251,154,153,',
    'rgba(51,160,44,',   'rgba(178,223,138,', 'rgba(31,120,180,',
    'rgba(166,206,227,',
    'rgba(177,89,40,',  // color list duplicates from here on.
    'rgba(106,61,154,',  'rgba(202,178,214,', 'rgba(253,191,111,',
    'rgba(227,26,28,',   'rgba(251,154,153,', 'rgba(51,160,44,',
    'rgba(178,223,138,', 'rgba(31,120,180,',  'rgba(166,206,227,',
    'rgba(177,89,40,',   'rgba(106,61,154,',  'rgba(202,178,214,',
    'rgba(253,191,111,', 'rgba(227,26,28,',   'rgba(251,154,153,',
    'rgba(51,160,44,',   'rgba(178,223,138,', 'rgba(31,120,180,',
    'rgba(166,206,227,'
...
var depthFirstTraversalProtocolTree =
    function() {
    var sortedList = [];
    ...
    for (var i = 0; i < protocolNames.length; i++) {
        ...
        if (level == 1 && !deleted) {
            ...
            protocolObject[protocolName].hasChildren =
                preOrderTraversal(protocolName, childrenList, protocolNames);
...
function preOrderTraversal(protocolName, childrenList, protocolNames) {
    ...
    for (var i = 0; i < protocolNames.length; i++) {
        ...
        if (parentName == protocolName && !deleted) {
            ...
            protocolObject[tempProtocolName].hasChildren = preOrderTraversal(
                tempProtocolName, childrenList, protocolNames);
            hasChildren = true;
        }
    }
    return hasChildren;
...
window.onload =
    ...
    */
    ...
    var fileTemp = $.ajax({
                        ...
                        url: '/receive_transcript_file',  // change to
                                                          // send_trn_fil
        ...
        for (var i in captionArray) {
            if ((captionArray[i].length > 1) &&
                ...
                if ((captionArray[i][2] !== 'F1' &&
                    ...
                    labelColor =
                        speakerColors[parseInt(captionArray[i][2].split('F')[1]) - 1];
                }
        ...
        var tagListHTML =
            makeWordList(lowerCaseLines, textMetadata, tagsToRemove, true);
        updateTagList($('#tagList'), tagListHTML);

        // Remove tag on right click
        var tagListDOM = $('#tagList');
        tagListDOM.oncontextmenu = function() {
            return false;
        };
        tagListDOM.on('mousedown', 'text', function(e) {
            if (e.button == 2) {
                var isRemoveTag =
                    confirm('Remove tag: ' + $(this).text() + ' from list?');
                if (isRemoveTag == true) {
                    var tagToRemove = $(this).text();
                    tagsToRemove.push(tagToRemove);
                    var tagListHTML = makeWordList(
                        lowerCaseLines, textMetadata, tagsToRemove, true);
                    updateTagList($('#tagList'), tagListHTML);
        ...
        tagListDOM.on('mouseenter', 'text', function() {
            ...
            var transFilter =
                $('#transTable')
                    .find('span:containsNC(\'' + tagHoverText + '\')');
            var transItems =
                transFilter
                    .filter(function() {
            ...
            if (isTagClicked) {
                ...
                transItems.each(function(index, value) {
                    ...
                    d3.select(hiRects[idIndex]).attr('fill', oldHighlighting);
        ...
        tagListDOM.on('click', 'text', function(e) {
            cTime = new Date();
            var tempTime = cTime.getHours() + ':' + cTime.getMinutes() + ':' +
                cTime.getSeconds();
            ...
            if (e.ctrlKey || e.metaKey) {
                ...
                */
                ...
                if (prevClickedTag !== tagHoverText) {
                    $(this).parent().children('text').removeClass(
                        'tagClickHighlight');
                    ...
                    transItems.each(function(index, value) {
                        ...
                        var hiRects =
                            $('#transGraphContent svg').children('rect');
        ...
        topicListDOM.on('mouseenter', 'tr', function() {
            ...
            for (var i = 0; i < topicHoverSpans.length; i++) {
                ...
                var transFilter =
                    $('#transTable')
                        .find('span:containsNC(\'' + topicHoverText + '\')');
                var transItems = transFilter
                                     .filter(function() {
                                         return (this.textContent)
                                             .toLowerCase()
                ...
                transItems.addClass('hoverHighlight');
                if (isRowClicked) {
                    // do nothing to the transGraph on mouseenter if a word in
                    // the tag cloud is already clicked.
                } else {
                    transItems.each(function(index, value) {
                        var idIndex = value.parentNode.rowIndex;
            ...
            if (prevClickedRow !== $(this).attr('id')) {
                ...
                for (var i = 0; i < topicHoverSpans.length; i++) {
                    ...
                    var transFilter =
                        $('#transTable')
                            .find(
                                'span:containsNC(\'' + topicHoverText + '\')');
                    var transItems = transFilter
                                         .filter(function() {
                                             return (this.textContent)
                                                 .toLowerCase()
                ...
                $(this).parent().children('tr').removeClass(
                    'tagClickHighlight');
                $('#transTable').find('td').removeClass('textClickHighlight');
                d3.select('#transGraphContent')
                    .selectAll('svg')
                    .selectAll('rect')
                    .data(transGraphData)
        ...
        $('#transTable').on('mouseenter', 'tr', function() {
            ...
            var transItemIds = [], idIndex = this.rowIndex;
            transItemIds.push(idIndex);
            // change color of text rep bars
            var hiRects = $('#transGraphContent svg').children('rect');
            d3.select(hiRects[idIndex]).classed('transRectHighLight', true);
            d3.select('#icGraphContent').selectAll('rect').each(function(d) {
                if (d.rowIndex === idIndex) {
                    d3.select(this).classed('transRectHighLight', true);
                }
            });
            d3.select('#tagGraphContent').selectAll('rect').each(function(d) {
        ...
        $('#transTable').on('mouseleave', 'tr', function() {
            ...
            d3.select('#transGraphContent svg')
                .selectAll('rect')
                .classed('transRectHighLight', false);
            d3.select('#icGraphContent svg')
                .selectAll('rect')
                .classed('transRectHighLight', false);
            d3.select('#tagGraphContent svg')
                .selectAll('rect')
                .classed('transRectHighLight', false);
        });
        ...
        toggleMinMax(
            'transGraphTitle', 'transGraph', 'Graphical View of Transcript',
            transGraphWidth);
        toggleMinMax(
            'icGraphTitle', 'icGraph', 'Graphical View of Information Content',
            icGraphWidth);
        toggleMinMax(
            'tagGraphTitle', 'icGraph', 'Graphical View of Tags',
            tagGraphWidth);
        ...
        $('ul.tabs li:first').on('click', function() {
            ...
            if ($(this).hasClass('active')) {
                ...
                var protoViewSvg =
                    d3.select('#protoView').append('svg').attr('height', 400);
                ...
                // note that this is a linear buffer but the hierarchy is
                // encoded in level variable
                ...
                for (var i = 0; i < newProtocolList.length; i++) {
                    ...
                    for (var j = 0; j < protocolKeyList.length; j++) {
                        ...
                        var templevel =
                            protocolObject[protocolKeyList[j]].level;
                        var tempParentName =
                            protocolObject[protocolKeyList[j]].parentName;
                        ...
                        if (tempProtocolName == protocolName) {
                            if (templevel == level) {
                                if (tempParentName == parentName) {
                                    // found the protocol .. now unsetting the
                                    // delete flag
                                    protocolObject[tempProtocolName].deleted =
                                        false;
                                    break;
                                } else {
                                    // renaming the parent name will just work
                                    protocolObject[tempProtocolName]
                ...
                var protoRect = protoViewSvg.selectAll('g')
                                    ...
                                    .attr('transform', function(d, i) {
                                        ...
                                        var yline =
                                            i * 16 + 10 + protoViewPadding;

                                        return 'translate(' + indents * 10 +
                                            ',' + yline + ')';
                                    });
        ...
        hmsToSec(selectedIndices[ind][1]); var timeInSecs =
        ...
        */
        ...
        $('#transTable').on('mouseup', function(e) {
            ...
            if (selectedText.length > 0) {
                ...
                if (e.button !== 2) {
                    ...
                    var tagListHTML = makeWordList(
                        linesList, textMetadata, tagsToRemove, false);
                    updateTagList($('#tagList'), tagListHTML);
                    ...
                    sendClickData.data = clickLog;
                    $.post(
                        '/clicklog', sendClickData, function(data, error) {});
                }
            } else {
                var tagListHTML = makeWordList(
                    lowerCaseLines, textMetadata, tagsToRemove, true);
        ...
        $('#transContent').on('contextmenu', function(e) {
            ...
            var bottomRightTopOffset = $('#center').offset().top;
            var bottomRightLeftOffset = $('#center').offset().left;
            var bottomRightWidth = $('#center').width();
            var bottomRightHeight = $('#center').height();
            var contextMenuWidth = $('.contextmenu').html(menuItems).width();
            var contextMenuHeight = $('.contextmenu').html(menuItems).height();
            if (pageXOffset + e.clientX <=
                ...
                if (pageYOffset + e.clientY <=
                    ...
                    menuXpos = pageXOffset + e.clientX;
            ...
            $('.contextmenu').html(menuItems).css({
                'visibility': 'visible',
                'left': menuXpos + 'px',
                'top': menuYpos + 'px',
                'background': 'white',
        ...
        $('.contextmenu').on('click', 'ul', function(evt) {
            ...
            if ($.contains(this, '#newCodeBtn')) {
                ...
                for (var i in orgSpanCollection) {
                    ...
                    if ($(this).text() === 'unassign') {
                        for (var ksel in selectedIndices) {
                            var lineIDCurrentSel =
                                $(spansList[0]).attr('id').split('word')[0];
                            var lineIDCoded =
                                selectedIndices[ksel][4][0].split('word')[0];
                        ...
                        for (var j in spansList) {
                            ...
                            tempSpan
                                .css({
                                    'background-color':
                                        protocolColorList[protocolList.indexOf(
                                            $(this).text())]
                                })
                                .delay(1000)
                                .animate(
                ...
                d3.select('#protocolGraphContent').selectAll('svg').remove();
                ...
                var protoY =
                    d3.scale.linear()
                        ...
                        .range(
                            [0, protoGraphHeight - margin.top - margin.bottom]);
                var protoX =
                    d3.scale.ordinal()
                        .domain(protocolList)  // convert ditto
                        .rangePoints(
                            [margin.left, protoGraphWidth - margin.right], 0);
                var proSpace = 10;
                ...
                for (var ind = 0; ind < selectedIndices.length; ind++) {
                    ...
                    d.x = (d.codeIndex * (protoGraphWidth - proSpace) /
                        ...
                        .on('click', function(d) {
                            if (d3.event.ctrlKey || d3.event.metaKey) {
                                ...
                                var tempTime = cTime.getHours() + ':' +
                                    ...
                                    cTime.getSeconds();
                                clickLog.push([
                                    tempTime, 'genCodeWordCloud', d.code + '\n'
                                ]);
                                sendClickData.data = clickLog;
                                $.post(
                                    '/clicklog', sendClickData,
                                    function(data, error) {});
                                //
                                var lineCollection = [];
                                if (clickStatus === 0) {
                                    ...
                                    var sameCodeObjs =
                                        $.grep(codedData, function(e) {
                                            return e.code == d.code;
                                        });
                                    // color all spans in these objects
                                    // persistently
                                    for (var ind = 0; ind < sameCodeObjs.length;
                                         ind++) {
                                        ...
                                        var codedWords =
                                            currentObj.transcriptLine
                                                .toLowerCase()
                                                .split(wordSeparators);
                                        lineCollection.push(codedWords);
                                        // exempt these rectangles from
                                        // mouseover, mouseout events.
                                        currentObj.clickStatus = 1;
                                    }
                                    var tagListHTML = makeWordList(
                                        lineCollection, textMetadata,
                                        tagsToRemove, false);
                                    updateTagList($('#tagList'), tagListHTML);

                                    // set general click status as 1, so that
                                    // this has to be disabled before another
                                    // group of spans can be permanently
                                    // highlighted.
                                    clickStatus = 1;
                                } else {
                                    $('#transTable').find('span').css({
                                        'background-color': 'rgba(0,0,0,0)'
                                    });
                                    for (var ind = 0; ind < codedData.length;
                                         ind++) {
                                ...
                                var tempTime = cTime.getHours() + ':' +
                                    ...
                                    cTime.getSeconds();
                                clickLog.push([
                                    tempTime, 'codeSkipToTime', d.startTime,
                                    d.code + '\n'
                                ]);
                                sendClickData.data = clickLog;
                                $.post(
                                    ...
                                    function(data, error) {});
                                //
                                // this small snippet below to scroll the
                                // transcript to show the line corresponding to
                                // the item selected in transgraph
    ...
    var fileTemp3 = $.ajax({
        ...
        icGraphData = generateICGraph(
            '#icGraphContent', captionArray, lowerCaseLines, sentenceTags,
            textMetadata);
    ...
    $('#showPeople').change(function() {
        ...
        generateMultiWordGraphs(
            '#tagGraphContent', captionArray, lowerCaseLines, sentenceTags,
            checkedLabels);
    });
    ...
    $('#showAdjectives').change(function() {
        if (!($(this).is(':checked'))) {
            ...
            tagLines(
                $('#transContent'), sentenceTags, 'adjective', 'adjectives');
            tagWordCloud(
                $('#tagList'), textMetadata, 'adjective', 'adjectives');
        }
        var checkedLabels = returnSelectedLabels();
        d3.select('#tagGraphContent').selectAll('svg').remove();
        generateMultiWordGraphs(
